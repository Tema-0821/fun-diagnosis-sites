...
export function LeadershipApp() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-1 flex-col items-center px-6 py-12">
      ...
      <h1 className="mt-3 text-center text-3xl font-bold tracking-tight text-sky-600">
        🧭 나의 리더십 스타일
      </h1>
      ...
      {!result ? (
        <form onSubmit={handleSubmit} className="mt-8 flex w-full flex-col">
          <div className="mb-6 h-1 w-full rounded-full bg-sky-100">
            <div
          ...
          {QUESTIONS.map((q, index) => (
            <div key={q.id} className="border-t border-sky-100 py-5 first:border-t-0 first:pt-0">
              <p className="text-sm text-zinc-800">
                <span className="mr-2 text-sky-400">{index + 1}.</span>
                {q.text}
              </p>
          ...
          <button
            ...
            className="mt-8 w-full rounded-2xl bg-gradient-to-r from-sky-400 to-blue-400 px-4 py-3 text-lg font-bold text-white shadow-md transition-transform hover:scale-[1.02] active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:scale-100"
          >
            결과 보기 🧭
          </button>
        </form>
      ) : (
        <div className="animate-pop-in mt-8 w-full rounded-3xl border border-sky-100 bg-white/90 p-6 text-center shadow-lg">
          <p className="text-6xl">{result.info.emoji}</p>
          <p className="mt-2 text-2xl font-extrabold text-sky-600">{result.info.name}</p>
          ...
          <div className="mt-6 flex gap-2">
            <button
              type="button"
              onClick={handleShare}
              className="flex-1 rounded-xl border border-sky-200 px-4 py-2.5 text-sm font-medium text-zinc-700 transition-colors hover:bg-sky-50"
            >
