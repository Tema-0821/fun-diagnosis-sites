...
import { decodeAnswers, encodeAnswers, generateJob, type JobResult } from "@/lib/job/generate";
import { QUESTIONS, type JobTag } from "@/lib/job/quiz";

export function JobApp() {
  const router = useRouter();
  const searchParams = useSearchParams();

  const [answers, setAnswers] = useState<Record<string, JobTag>>({});
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const next = generateJob(answers);
    if (!next) return;
    setResult(next);
    setCopied(false);
    router.replace(`/job?a=${encodeAnswers(answers)}`);
  }
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-1 flex-col items-center px-6 py-12">
      ...
      <p className="mt-2 text-center text-sm text-zinc-400">
        질문 {QUESTIONS.length}개로 알아보는 나에게 맞는 판타지 직업
      </p>
      ...
      {!result ? (
        ...
        <div className="card-tarot animate-card-reveal mt-6 w-full rounded-xl p-6 text-center">
          <p className="text-xs tracking-[0.2em] text-purple-300 uppercase">직업 적성 테스트</p>
          ...
          <p className="mt-3 text-left text-sm text-purple-300">💪 {result.info.strength}</p>
